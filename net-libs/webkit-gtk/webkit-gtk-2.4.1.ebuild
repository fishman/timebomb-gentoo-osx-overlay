...
LICENSE="LGPL-2+ BSD"
SLOT="2"
KEYWORDS="~alpha ~amd64 ~arm ~ia64 ~ppc ~ppc64 ~sparc ~x86 ~amd64-fbsd ~x86-fbsd ~x86-freebsd ~amd64-linux ~ia64-linux ~x86-linux ~x86-macos ~x64-macos"
...
src_prepare() {
	...
	if use x64-macos; then
		...
		sed -i "/^#define USE_ACCELERATE_FFT/s/^.*$/#define USE_ACCELERATE_FFT 0/" \
			"${S}"/Source/WebCore/platform/audio/FFTFrame.h
	fi
	...
	# fix darwin build
	epatch "${FILESDIR}/${PN}-2.4.1-darwin.patch"
