...
IUSE="context debug doc icu +nls mpi python static-libs +threads tools clang"
...
#	epatch "${FILESDIR}"/${PN}-1.37.0-darwin-long-double.patch
    ...
    if use clang; then
        epatch \
            "${FILESDIR}/${PN}-1.55.0-clang-support.patch" \
            "${FILESDIR}/${PN}-1.55.0-clang-support2.patch"
    fi
}
